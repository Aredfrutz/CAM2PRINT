...
final TSPrintController = TextEditingController(text: TSPrintInitialValue.toStringAsFixed(2));
final TSXeroxController = TextEditingController(text: TSXeroxInitialValue.toStringAsFixed(2));
final TSSuppliesController = TextEditingController(text: TSSuppliesInitialValue.toStringAsFixed(2));
final TSPartyController = TextEditingController(text: TSPartyInitialValue.toStringAsFixed(2));
final CAmountController = TextEditingController(text: CAmountInitialValue.toStringAsFixed(2));
final DViolationsController = TextEditingController(text: DViolationsInitialValue.toStringAsFixed(2));
final DPhilHealthController = TextEditingController(text: DPhilHealthInitialValue.toStringAsFixed(2));
final DSSSController = TextEditingController(text: DSSSInitialValue.toStringAsFixed(2));
final LDeductionController = TextEditingController(text: LDeductionInitialValue.toStringAsFixed(2));
final CAdvanceController = TextEditingController(text: CAdvanceInitialValue.toStringAsFixed(2));
final NRemindersController = TextEditingController(text: "");

final TSPrintPercentageController = TextEditingController(text: TSPrintPercentageInitialValue.toStringAsFixed(0));
final TSXeroxPercentageController = TextEditingController(text: TSXeroxPercentageInitialValue.toStringAsFixed(0));
final TSSuppliesPercentageController = TextEditingController(text: TSSuppliesPercentageInitialValue.toStringAsFixed(0));
final TSPartyPercentageController = TextEditingController(text: TSPartyPercentageInitialValue.toStringAsFixed(0));
...
enum IsFullTimeStaff {
  ...
  static final List<StaffBool> entries = UnmodifiableListView<StaffBool>(
    values.map<StaffBool>(
      (IsFullTimeStaff boolean) => StaffBool(value: boolean, label: boolean.label),
    ),
  );
}
...
class _SalaryPageState extends State<SalaryPage> with SingleTickerProviderStateMixin {
  
  // NEW: Variables for dynamic staff data from Supabase
  List<Map<String, dynamic>> _staffList = [];
  String? _selectedStaffId;
  String _selectedStaffName = 'Loading...';
  ...
  // ✅ FIXED initState() - ONLY ONE, PROPERLY CLOSED
  @override
  void initState() {
    super.initState();
    _fetchStaffFromSupabase();
    resetTextFields();
    tabController = TabController(initialIndex: 0, length: 2, vsync: this);
    
    // All your listeners go HERE, inside initState:
    TSPrintController.addListener(TSPrintState);
    TSXeroxController.addListener(TSXeroxState);
    TSSuppliesController.addListener(TSSuppliesState);
    TSPartyController.addListener(TSPartyState);
    CAmountController.addListener(CAmountState);
    DViolationsController.addListener(DViolationsState);
    DPhilHealthController.addListener(DPhilHealthState);
    DSSSController.addListener(DSSSState);
    LDeductionController.addListener(LDeductionState);
    CAdvanceController.addListener(CAdvanceState);
    
    NRemindersController.addListener(() {
      setState(() => NRemindersValue = NRemindersController.text);
    });
    
    TSPrintPercentageController.addListener(TSPrintPercentageState);
  ...
  // Formula para macompute Salary - ✅ SAME CALCULATIONS, just using _selectedIsFullTime
  void formula() {
    if (_selectedIsFullTime) { // ✅ Changed from: staffBoolean == IsFullTimeStaff.yes
      BasicPay =
                              ...
                              children: [
                                Column(
                                  children: [
                                    ...
                                    SizedBox(height: 4),
                                    // ✅ UPDATED: Use _selectedIsFullTime
                                    if (_selectedIsFullTime)
                    ...
                    children: [
                        ...
                        child: Column(
                          children: [
                            ...
                            _isLoadingStaff
                                ...
                                : DropdownMenu<String>(
                                    dropdownMenuEntries: _staffList.map((staff) {
                                    ...
                                    initialSelection: _selectedStaffId,
                                    ...
                                    onSelected: (String? staffId) {
                                      setState(() {
                                        ...
                                        _selectedIsFullTime = (selectedStaff['employment_status'] ?? '')
                                            .toLowerCase()
                                            .contains('full');
                      ...
                      Flexible(
                        ...
                        child: Column(
                          children: [
                            SizedBox(height: 24),
                            // ✅ CHANGED: View-only box, white text, no arrow
                            Container(
                              height: 48,
                              ...
                              child: Text(
                                _selectedIsFullTime ? 'Full-time Staff' : 'Part-time Staff',
                                style: TextStyle(
