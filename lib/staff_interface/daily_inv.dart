...
@override
  ...
  try {
    ...
    // 1. Fetch profile (for name/role display only)
    ...
    // ✅ 2. Fetch TODAY's assigned branch from branch_assignments
    ...
    // ✅ Use assigned branch for TODAY, fallback to profile default
    final branchName = assignment?['branch_name'] ?? profileData['assigned_branch'] ?? 'Main Branch';
    ...
    // 4. Fetch Inventory for the DYNAMIC branch
    final inventoryData = await Supabase.instance.client
        .from('inventory')
        .select()
        .eq('branch_name', branchName) // ✅ Now uses dynamic branch
...
Future<void> _fetchHistory({DateTime? filterDate}) async {
  ...
  try {
    ...
    // ✅ 1. Get the CURRENT user's assigned branch (for TODAY)
    // This determines which branch's history to show
    final today = DateFormat('yyyy-MM-dd').format(DateTime.now());
    final userId = user.id;
    ...
    final currentBranchName = currentAssignment?['branch_name'];
    if (currentBranchName == null) {
      debugPrint('❌ No branch assigned for today');
      setState(() {
        _inventoryItems = [];
        _isLoading = false;
      });
    ...
    // ✅ 2. Fetch history for the CURRENT branch
    var query = Supabase.instance.client
        .from('inventory_history')
        .select()
        .eq('branch_name', currentBranchName); // Filter by branch directly
    ...
    if (filterDate != null) {
      ...
      debugPrint('🔍 Filtering from: $startOfDay to: $endOfDay');
    ...
    debugPrint('📊 Fetched ${data.length} records for branch: $currentBranchName');
...
Future<void> _saveAndReset() async {
  ...
  try {
    // ✅ 1. Fetch the current user's assigned branch for TODAY
    final user = Supabase.instance.client.auth.currentUser;
    ...
    final branchName = assignment?['branch_name'];
    if (branchName == null) {
      throw Exception('No branch assigned for today');
    }
    
    // ✅ 2. Now process each inventory item
    for (var item in _inventoryItems) {
      ...
      // Only proceed if the user actually changed the numbers
      if (oldPacks != item['packs'] || oldPieces != item['pieces']) {
        ...
        // ✅ Insert the log into history WITH branch_name
        await Supabase.instance.client.from('inventory_history').insert({
          'inventory_id': item['id'],
          'item_name': item['item_name'],
          'branch_name': branchName, // ✅ Now defined!
          'old_packs': oldPacks,
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: Row(
          ...
          _fetchInventoryFromSupabase(); // Refresh inventory data
        },
        child: Container(
          padding: const EdgeInsets.symmetric(
            horizontal: 20,
          ...
          decoration: BoxDecoration(
            color: const Color(0xFF5C6BC0), // Different color to distinguish from Filter
            borderRadius: BorderRadius.circular(20),
          ),
          child: const Row(
                          ...
                          GestureDetector(
                            onTap: () async {
                              if (_currentTab == 'inventory') {
                                _toggleEditInventory();  // ✅ Added underscore
                                } else {
                                await _saveAndReset();
                                }
                              },
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          isDense: true, // ✅ Add this to minimize default padding
        ),
        onChanged: (value) {
          setState(() {
            item['reason'] = value;
